...
class LatentManager:
    ...
    def get_decode_fn(self):
        ...
        # cost_analysis() returns a list on jax<=0.4 and a dict on jax>=0.5.
        try:
            ca = compiled_decod_fn.cost_analysis()
            if isinstance(ca, (list, tuple)):
                ca = ca[0]
            log_for_0(f"FLOPs (1e9): {ca['flops'] / 1e9}")
        except (KeyError, IndexError, TypeError):
            log_for_0('FLOPs: unavailable on this jax version')
