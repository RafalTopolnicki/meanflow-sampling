...
def compute_metrics(dict_losses):
  ...
  metrics = jax.tree_util.tree_map(lambda x: x.flatten(), metrics)  # (batch_size,)
  return metrics
...
def train_and_evaluate(
  ...
  if config.eval_only:
    # Eval-only needs no training data; skip building the (huge) latent loader.
    train_loader, steps_per_epoch = None, 1
  else:
